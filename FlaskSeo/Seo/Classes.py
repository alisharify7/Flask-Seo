...
class SeoFlasky(OriginalFlask):
    ...
    def route(self, rule:str, SeoCheck:bool=True, SeoForce:bool=False, UserAgent:str="*", SitemapCheck:bool=True, SitemapForce:bool=False, Sitemappriority:int=1,*args, **kwargs):
        ...
        if self.config.get('ROBOTS_TXT', False):
            ...
            if re.search("<\w+\:\w+>", rule) and not SeoForce:
                WarningLogger.warning(f"Url `{rule}` by security reassons is not added to robots.txt file. Use `SeoForce=True` to Force it to adding in robots.txt")
                return content
                ...
                case "default":
                    if SeoCheck:
                        if not self.config.get(Values.SEO_ALLOW, False):
                            ...
                            self.config[Values.SEO_ALLOW].add(json.dumps(data))
                        else:
                            self.config[Values.SEO_ALLOW].add(json.dumps(data))
                    else:
                        if not self.config.get(Values.SEO_DISALLOW, False):
                            self.config[Values.SEO_DISALLOW] = set()
                            self.config[Values.SEO_DISALLOW].add(json.dumps(data))
                        else:
                            self.config[Values.SEO_DISALLOW].add(json.dumps(data))
                case "redis":
                    self.connection.set(name=f"FLASK_SEO:{rule}", value=json.dumps(data))
    ...
    def set_seo_connection(self, connection_type:str, connection=None):
        ...
        if connection_type == 'default':
            ...
            WarningLogger.warning("Note: Seo Type is `default`, this connection is not for production enviroment.Use this for development only. Use Redis.redis connection for production")
        else:
    ...
    def __init__(self, *args, **kwargs):
        ...
        def robots_txt_view():
            ...
            message = f"# Auto Generated robots.txt\n# Version:2.0.0\t{datetime.datetime.utcnow()}\n\n\n"
            ...
            if self.connection_type == "default":
                ...
                for each in self.config.get(Values.SEO_ALLOW, {}):
                    ...
                    if each['url'] == '/robots.txt':
                        continue
                    record = (f"Allow: " if each['status'] else 'Disallow') + f"{each['url']}\n"
                ...
                for each in self.config.get(Values.SEO_DISALLOW, {}):
                    ...
                    if each['url'] == '/robots.txt':
                        continue
                    record = (f"Allow: " if each['status'] else f"Disallow: ") + f"{each['url']}\n"
                    if not records.get(each['agent'], False):
                        records[each['agent']] = [record]
                    else:
                        records[each['agent']].append(record)
...
def FlaskSeo(app:OriginalFlask, name:str):
    """ Wrapper Class For SeoFlasky Class """
